...
export function Table({ data }: { data: Color[] }) {
  ...
  return (
    <Card
      ...
      description="Column Sizing with useTable and tableFeatures"
    >
...
function ColumnResizingTableHeaderCell({
  children,
  width,
  onResize,
}: {
  ...
  return (
    <TableHeaderCell
      className="relative w-(--header-cell-size) truncate"
      style={
        {
          '--header-cell-size': `${width}px`,
        } as React.CSSProperties
      }
    >
      {children}
